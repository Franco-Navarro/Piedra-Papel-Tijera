...
// CARGA LAS RONDAS EN EL HEADER Y EL BOTON DE LA VUELTA ATRAS
function titleRounds() {
  if(game.actualRound < $inputRounds.value){
    ...
    game.actualRound += 1
    document.getElementById("header-game").innerHTML = '<div onclick="backMenu()"><svg width="44" height="37" viewBox="0 0 44 37" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M43.2 15.7349H8.98065L21.3982 3.3173L18.0809 0L0 18.0809L18.0809 36.1619L21.3982 32.8446L8.98065 20.427H43.2V15.7349Z" fill="white"/></svg></div><h2>FIN DEL JUEGO</h2>'
  }
}
...
function showRound() {
  ...
  if(winnerRound(cpu) == 1){
    ...
    roundWinner = '<div class="game-round-winner"><p>' + game.playerName + ' GANA</p></div>'
  }
  else if(winnerRound(cpu) == -1) {
    game.cpuWins += 1;
     handDIV = '<div class="game-selected">' + hand(game.playerHand) + svgVS + hand(cpu) + "</div>";
     roundWinner = '<div class="game-round-winner"><p>CPU GANA</p></div>'
   }
  else {
    game.same += 1;
    handDIV = '<div class="game-selected">' + hand(game.playerHand) + svgVS + hand(cpu) + '</div>';
    roundWinner = '<div class="game-round-winner"><p>EMPATE</p></div>'
  }

  

  statsDIV = '<div class="game-stats"><p>' + game.playerName + '</p><p>' + game.playerWins + ' | ' + game.cpuWins + '</p><p>CPU</p></div>';  
  document.getElementById("container-game").innerHTML = roundWinner + statsDIV + handDIV;
}
...
function showWinner(){
  ...
  if(game.playerWins > game.cpuWins) {
    $resultBoard.innerHTML = '<h2>' + game.playerName + ' ES EL GANADOR</h2>' + '<div class="result-board-score"><p>' + game.playerName + ' - ' + game.playerWins + '</p><p>CPU - ' + game.cpuWins + '</p><p>EMPATES - ' + game.same + '</p></div>'
  }
  else if(game.playerWins < game.cpuWins){
    $resultBoard.innerHTML = '<h2>CPU ES EL GANADOR</h2>' + '<div class="result-board-score"><p>' + game.playerName + ' - ' + game.playerWins + '</p><p>CPU - ' + game.cpuWins + '</p><p>EMPATES - ' + game.same + '</p></div>'
  }
  else {
    $resultBoard.innerHTML = '<h2>ES UN EMPATE</h2>' + '<div class="result-board-score"><p>' + game.playerName + ' - ' + game.playerWins + '</p><p>CPU - ' + game.cpuWins + '</p><p>EMPATES - ' + game.same + '</p></div>'
  }
...
function backMenu() {
  ...
  (game.playerName = " "), (game.playerHand = -1), (game.playerWins = 0), (game.cpuWins = 0), (game.actualRound = 0),(game.same = 0);
  playerSelection(-1); 
